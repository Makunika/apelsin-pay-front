...
import {fHeaderDeposit, fNumberDeposit} from "../../../utils/formatNumber";
...
function TransactionItemDetail({ item, isDeposit }) {
  ...
  return (
    <Stack justifyContent="flex-start" direction="column">
      <Box mb={2} >
        <Typography variant="subtitle1">{fHeaderDeposit(isDeposit, type === 'PAYMENT')}</Typography>
      </Box>
      <SimpleDataVisible
        label="Дата платежа"
      ...
      {tinkoffPayUrl && !(isDeposit && type === 'PAYMENT') && status === 'WAIT' && (
